 ...
 */
const PROFILE_VERSION = "1788750000"; // 07.09.2026, третья редакция
...
const ROUTING_PROFILE = {
  ...
  // ВАЖНО. `AsIs` — имя сайта уходит в тоннель как есть, и разрешает его наш
  // сервер в Нидерландах. Любая другая стратегия заставляет приложение сначала
  // выяснить адрес на месте, а в России ответы DNS для заблокированных
  // ресурсов подменяют: приложение получает мёртвый адрес и идёт по нему даже
  // через тоннель. Ровно это и наблюдалось 07.09.2026: российские сайты и
  // незаблокированные открывались, а Telegram нет.
  DomainStrategy: "AsIs",
  FakeDNS: "false",
};
